# Quick test: call the gRPC gateway using a minimal .NET script
# This builds and runs a tiny console app that tests the gateway
...
string[] apiPaths = [
    ...
    "api/stock/capitalflow/ticker?tickerId=913254235&showDm=false",
...
foreach (var apiPath in apiPaths)
{
    var req = new ClientRequest
    {
        ...
        Path = ByteString.CopyFromUtf8(apiPath),
    };
    ...
    try
    {
        ...
        Console.WriteLine($"[{body.Length}B] {apiPath}");
        Console.WriteLine($"  {preview}");
    }
    catch (Exception ex)
    {
        Console.WriteLine($"[ERR] {apiPath}: {ex.Message}");
    }
}
'@

# Write temp project
$tmpDir = "$env:TEMP\grpc_test_$(Get-Random)"
